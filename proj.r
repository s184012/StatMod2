...
p = mod15$rank;
n = length(DIOX);
plot( mod15$fitted.values, lev, ylab = "Leverages", main = "Plot of Leverages",
      pch = 16, col = 'black' )
...
# RESIDUAL ANALYSIS
gs = summary(mod15)
res_std = mod15$residuals/sd(DIOX)
watchout_ids_rstd = which( abs( res_std ) > 2 )
watchout_rstd = res_std[ watchout_ids_rstd ]
watchout_rstd
plot( mod15$fitted.values, res_std, ylab = "Standardized Residuals", main = "Standardized Residuals" )
abline( h = c(-2,2), lty = 2, col = 'orange' )
points( mod15$fitted.values[watchout_ids_rstd],
        res_std[watchout_ids_rstd], col = 'red', pch = 16 )
points( mod15$fitted.values[watchout_ids_lev],
...
plot( mod15$fitted.values, stud, ylab = "Studentized Residuals", main = "Studentized Residuals", pch = 16 )
points( mod15$fitted.values[watchout_ids_stud],
...
# Cook Distance
Cdist = cooks.distance( mod15 )
watchout_ids_Cdist = which( Cdist > 4/(n-p) )
watchout_Cdist = Cdist[ watchout_ids_Cdist ]
watchout_Cdist
if(saveFig == TRUE){pdf("Influe.pdf", width = 10*0.8, height = 10*0.8)}
par( mfrow = c( 1, 3 ) )
plot( mod15$fitted.values, Cdist, pch = 16, xlab = 'Fitted values',
      ylab = 'Cooks Distance', main = 'Cooks Distance' )
points( mod15$fitted.values[ watchout_ids_Cdist ], Cdist[ watchout_ids_Cdist ],
        col = 'green', pch = 16 )
plot( mod15$fitted.values, stud, pch = 16, xlab = 'Fitted values',
      ylab = 'Studentized Residuals', main = 'Studentized Residuals' )
points( mod15$fitted.values[ watchout_ids_stud ], stud[ watchout_ids_stud ],
        col = 'pink', pch = 16 )
plot( mod15$fitted.values, lev, pch = 16, xlab = 'Fitted values',
...
gl = lm( log(DIOX) ~ PLANT + TIME + LAB + O2COR + NEFFEKT + SO2 + HCL, dat[ id_to_keep, ] )
summary( gl )
par(mfrow=c(2,2))
logLik(gl)
plot(gl)
